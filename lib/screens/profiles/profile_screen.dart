...
import 'package:flutter_image_stack/flutter_image_stack.dart' as ImageStack;
import 'package:shared_preferences/shared_preferences.dart';
import '../../instruments/api.dart';
import '../../instruments/components/connection_checker_widget.dart';
import '../../instruments/components/events/list_event_item.dart';
import '../../instruments/components/profiles/list_profile_item.dart';
  ...
  void fetchData(){
    ...
    user.then((val) {
      setState(() {
        ...
        profilePicture = val.getProfilePicture();
        friends = _apiService.getFriendList(val.id);
                                ...
                                builder: (BuildContext ctx, AsyncSnapshot snapshot) {
                                  if (snapshot.connectionState != ConnectionState.done) {
                                    ...
                                    if(snapshot.data.length==0){
                                      ...
                                      List<User> friendList = snapshot.data;
                                      return GestureDetector(
                                        onTap: (){
                                          ...
                                          Navigator.push( context, MaterialPageRoute(
                                            builder: (context) => ShowAllPageLayout(
                                              items: friendList,
                                              appBarText: 'Друзья',
                                              buildItem: (friend, index){
                                                return ListProfileItem(
                                                  user: friend,
                                                  onTap: (){
                                                    goToFriendPage(friend.id);
                                                  },
                                                );
                                              },
                                            ),
                                        ...
                                        child: Row(
                                          ...
                                          children: [
                                            Text(
                                              '${friendList.length} ${friendsRightCase(friendList.length)}',
                                              style: kTextStyle,
                                            ),
                                            ImageStack.FlutterImageStack.providers(
                                              providers: [
                                                friendList[0].getProfilePicture(),
                                                if(friendList.length>1)
                                                  friendList[1].getProfilePicture(),
                                                if(friendList.length>2)
                                                  friendList[2].getProfilePicture(),
                                              ],
                                              totalCount: friendList.length > 2 ? 3
                                                  : friendList.length > 1 ? 2
                                                  : 1,
                                              itemRadius: 50,
                                            ),
                                          ],
